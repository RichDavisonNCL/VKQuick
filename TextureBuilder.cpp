...
UniqueTexture TextureBuilder::BuildFromData(void* dataSrc, size_t byteCount, const std::string& debugName) {
...
void TextureBuilder::FinaliseTexture(const std::string& debugName, vk::CommandBuffer& usingBuffer, TextureJob& job, UniqueTexture& t) {
    if (m_generateMips && t->m_createInfo.mipLevels > 1) {
...
UniqueTexture TextureBuilder::BuildCubemapFromFile(
...
UniqueTexture TextureBuilder::BuildCubemap(const std::string& debugName) {
    UniqueTexture tex = GenerateTexture(m_cmdBuffer, m_requestedSize, true, debugName);
...
UniqueTexture	TextureBuilder::GenerateTexture(vk::CommandBuffer m_cmdBuffer, vk::Extent3D dimensions, bool isCube, const std::string& debugName) {
    Texture* t = new Texture();

    uint32_t mipCount = Texture::GetMaxMips(dimensions);
    ...
    t->m_createInfo = vk::ImageCreateInfo()
        .setImageType(dimensions.depth > 1 ? vk::ImageType::e3D : vk::ImageType::e2D)
        .setExtent(dimensions)
        .setFormat(m_format)
	...
	if (isCube) {
        t->m_createInfo.setFlags(vk::ImageCreateFlagBits::eCubeCompatible);
	}

    t->m_image = m_memManager->CreateImage(t->m_createInfo);
...
void TextureBuilder::UploadTextureData(vk::CommandBuffer m_cmdBuffer, TextureJob& job) {
    ...
    Buffer stagingBuffer = m_memManager->CreateStagingBuffer(allocationSize, "Staging Buffer");
    ...
    QuickVK::UploadTextureData(m_cmdBuffer, stagingBuffer, job.image, vk::ImageLayout::eUndefined, job.endLayout,
        vk::BufferImageCopy{
            ...
            .imageExtent{job.dimensions.width, job.dimensions.height, job.dimensions.depth},          
        }
    );
