...
void Texture::GenerateMipMaps(vk::CommandBuffer  buffer, vk::ImageLayout startLayout, vk::ImageLayout endLayout, vk::PipelineStageFlags2 endFlags) {
	for (int layer = 0; layer < m_createInfo.arrayLayers; ++layer) {
		...
		blitData.srcOffsets[1] = vk::Offset3D(m_createInfo.extent.width, m_createInfo.extent.height, 1);

		blitData.dstOffsets[0] = vk::Offset3D(0, 0, 0);
		blitData.dstOffsets[1] = vk::Offset3D(m_createInfo.extent.width, m_createInfo.extent.height, 1);

		for (uint32_t mip = 1; mip < m_createInfo.mipLevels; ++mip) {
			blitData.dstSubresource.setAspectMask(vk::ImageAspectFlagBits::eColor)
				.setMipLevel(mip)
				.setBaseArrayLayer(layer)
				.setLayerCount(1);

			blitData.dstOffsets[1].x = std::max(m_createInfo.extent.width >> mip, 1u);
			blitData.dstOffsets[1].y = std::max(m_createInfo.extent.height >> mip, 1u);
		...
		ImageTransitionBarrier(buffer, m_image, 
				...
				0, m_createInfo.mipLevels, layer, 1);
	}
}
